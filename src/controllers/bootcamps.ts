...
export const createBootcamp = asyncHandler(
  async (
    ...
    const publishedBootcamp = await Bootcamp.findOne({ owner: loggedUser.id });
    if (publishedBootcamp && loggedUser.role !== 'admin') {
      return next(
        new ErrorResponse(
          `The user with id ${loggedUser.id} has already published a bootcamp`,
          400
        )
      );
    }
...
export const updateBootcamp = asyncHandler(
  async (
    ...
    if (
      ...
      return next(
        new ErrorResponse(
          `User ${loggedUser.id} is not authorized to update this bootcamp`,
          401
        )
      );
    }
...
export const deleteBootcamp = asyncHandler(
  async (req: Request<{ id: BootcampDocument['id'] }>, res: Response, next) => {
    const bootcamp = await Bootcamp.findById(req.params.id);
    if (!bootcamp) {
      return next(
        new ErrorResponse(`Bootcamp not found with id of ${req.params.id}`, 400)
    ...
    if (
      ...
      return next(
        new ErrorResponse(
          `User ${loggedUser.id} is not authorized to delete this bootcamp`,
          401
        )
      );
    }
